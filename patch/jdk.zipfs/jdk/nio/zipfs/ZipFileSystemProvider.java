...
public class ZipFileSystemProvider extends FileSystemProvider {
    ...
    public FileSystem getFileSystem(URI uri) {
        synchronized (filesystems) {
            ZipFileSystem zipfs = filesystems.get(xboxRealPath(uriToPath(uri)));
            if (zipfs == null)
    ...
    private static Path xboxRealPath(Path p) {
        try {
            return p.toRealPath();
        } catch (IOException e) {
    ...
    void removeFileSystem(Path zfpath, ZipFileSystem zfs) throws IOException {
        synchronized (filesystems) {
            filesystems.remove(xboxRealPath(zfpath), zfs);
        }
    }
}
